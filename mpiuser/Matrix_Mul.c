...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		( cudaMalloc( (void **)&DeviceMyVectorA, ScatterSize * sizeof(float) ) );
		( cudaMalloc( (void **)&DeviceMyVectorB, ScatterSize * sizeof(float) ) );
		( cudaMalloc( (void **)&DeviceTempVector, ScatterSize * sizeof(float) ) );
		( cudaMalloc( (void **)&DeviceNodeSum, sizeof(float) ) );

		//Copying data from Host to device memory
		( cudaMemcpy( (void *)DeviceMyVectorA, (void *)MyVectorA, ScatterSize * sizeof(float), cudaMemcpyHostToDevice ) );
		( cudaMemcpy( (void *)DeviceMyVectorB, (void *)MyVectorB, ScatterSize * sizeof(float), cudaMemcpyHostToDevice) );
	...
	( cudaFree(DeviceMyVectorA) );
	( cudaFree(DeviceMyVectorB) );
	( cudaFree(DeviceTempVector) );
	( cudaFree(DeviceNodeSum) );	
